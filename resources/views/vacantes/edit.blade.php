...
<form class="max-w-lg mx-auto my-10" action="{{ route('vacantes.update',['vacante' => $vacante->id]) }}" method="POST">
    ...
    <div class="mb-5">
        ...
        <input id="titulo" type="text"
            ...
            name="titulo" placeholder="Título de la vacante" value="{{ $vacante->titulo }}">
        @error('titulo')
    ...
    <div class="mb-5">
        <label for="skils" class="block text-gray-700 text-sm mb-5">Habilidades y Conocimientos: <span
                class="text-xs">(Elige al menos 3)</span></label>
        @php
            $skills = ['HTML5', 'CSS3', 'CSSGrid', 'Flexbox', 'JavaScript', 'jQuery', 'Node', 'Angular', 'VueJS',
            'ReactJS', 'React Hooks', 'Redux', 'Apollo', 'GraphQL', 'TypeScript', 'PHP', 'Laravel', 'Symfony', 'Python',
            'Django', 'ORM', 'Sequelize', 'Mongoose', 'SQL', 'MVC', 'SASS', 'WordPress', 'Express', 'Deno', 'React
            Native', 'Flutter', 'MobX', 'C#', 'Ruby on Rails']
        @endphp
        <lista-skills :skills="{{ json_encode($skills) }}"
            :oldskills="{{ json_encode($vacante->skills) }}"></lista-skills>
        @error('skills')
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mt-3 mb-6"
                role="alert">
                <strong class="font-bold">Error!</strong>
    ...
    <button
        ...
        type="submit">Publicar Vacante</button>
</form>
@endsection
@section('scripts')
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Agrega al input hidden lo que el usuario escribe a medium editor
